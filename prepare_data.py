...
class DataConstructor(object):
    ...
    def compute_distance_nearby(self):
        ...
        self.embedings_data[0]["previous_distance"], self.embedings_data[length - 1]["next_distance"] = -1, -1
        for i in tqdm(range(length - 1)):
            data = self.embedings_data[i]
            next_data = self.embedings_data[i + 1]
            distance = float(data["embeddings"] @ next_data["embeddings"].T)
    ...
    def milvus_init(self, milvus_info: MilvusInfo) -> None:
        ...
        milvus_utils.init_milvus(
            embeding_model_name=config.select_embeddings_model.model_name,
            milvus_info=self.milvus_info,
        )

    def milvus_upsert(self, collection_name, batch_size: int) -> None:
        for i in tqdm(range(0, len(self.embedings_data), batch_size)):
            milvus_utils.upsert(collection_name=collection_name, data=self.embedings_data[i : i + batch_size])
    ...
    def elasticsearch_create_index(self):
        self.elasticsearch.create_index(self.elasticsearch_index, config.elasticsearch["body"])
    ...
    def elasticsearch_update_distance(self, batch_size: int) -> None:
        for i in tqdm(range(0, len(self.json_array), batch_size)):
            self.elasticsearch.bulk_update_distance(index=self.elasticsearch_index, data=self.embedings_data[i : i + batch_size])
...
if __name__ == "__main__":
    ...
    data_constructor.elasticsearch_init(
        esInfo=config.elasticsearch["info"],
        index=config.elasticsearch["index"],
        body=config.elasticsearch["body"],
    )
    data_constructor.elasticsearch_create_index()
    ...
    print("compute distance nearby")
    data_constructor.compute_distance_nearby()

    print("update distance")
    data_constructor.elasticsearch_update_distance(batch_size=500)
    ...
    collection_name = config.milvus[config.select_embeddings_model.model_name]["collection_name"]
